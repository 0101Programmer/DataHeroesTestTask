...
with conn:
    with conn.cursor() as cursor:
        ...
        # выбор колонок "event_id" и "is_attend"
        sort_by_query = '''SELECT event_id, is_attend FROM test_task_table;'''
        # --- --- ---

        # создание и группировка датафрейма на основе выбранных данных
        df = pd.read_sql_query(sort_by_query, engine)
        grouped_df = df.groupby("event_id", dropna=False).sum()
        print(grouped_df)
        # --- --- ---

        # визуализация итогового датафрейма
        plt.bar(grouped_df.index.astype(str), grouped_df["is_attend"], color=['green'])
        plt.ylabel("Посетили мероприятие (чел.)")
        plt.xlabel("id мероприятия")
        plt.tight_layout()
